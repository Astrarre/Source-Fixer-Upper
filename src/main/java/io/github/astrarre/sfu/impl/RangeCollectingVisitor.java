...
public class RangeCollectingVisitor extends TreeScanner<Void, Void> {
    ...
    public Void visitClass(ClassTree node, Void unused) {
        ...
        for (Tree member : node.getMembers()) {
            if (member instanceof VariableTree v) {
                JCTree jc = (JCTree) v.getType(); // wait no this wont work, fuck, only really works for single variable
                                                  // declarations
    ...
    public Void visitIdentifier(IdentifierTree node, Void unused) {
        ...
        if (mirror != null) {
            ...
            this.types.add(new TypeRange(tree.getStartPosition(), tree.getEndPosition(this.root.endPositions),
                    DescVisitor.getDesc(mirror), false));
        }
    ...
    public Void visitMemberSelect(MemberSelectTree node, Void unused) {
        if (this.methodLock) {
            // Class.staticMethod & Outer.this.instanceMethod can be calculated here, maybe store if method is static as
            // heuristic?
            return null;
        }
    ...
    void addTypeRange(Tree tree) { // todo find "roots" (eg. array element types, each of the parameter types, so on so
                                   // forth), maybe just skip them and visit?
        if (tree == null)
            return;
